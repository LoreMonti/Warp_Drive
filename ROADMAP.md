...
Status of the `warpdrive` package. Checked items are implemented and covered
by the test suite; unchecked ones are planned.
...
- [x] **Causal structure** — horizon solver for $\beta + c/B = v_s$, reducing
      to $f = 1 - c/v_s$ for Alcubierre.
...
- [ ] Rendered star field at several $v_s$
...
4D spacetime rather than reduced to an effective potential. With lapse
...
- for $v_s > c$ the horizon computed by `horizon_offset` shows up as a region
  no backwards ray can reach — a black disc ahead of the ship. That is a useful
  check on the integrator: the disc must appear exactly where the bisection
  puts it.
...
- [ ] Re-derived expansion and energy density
...
$$ds^2 = -c^2 dt^2 + B(r_s)^2\left[\left(dx - v_s f(r_s)\,dt\right)^2 + dy^2 + dz^2\right]$$
...
- add the $B$ profile to `shapes.py` and a `BroeckMetric` in `metrics/`
  overriding `conformal_factor`; the interior is no longer conformally flat, so
  `expansion` and `energy_density` must be re-derived rather than inherited —
  $B$ and its first two derivatives both enter $T_{\mu\nu}$;
